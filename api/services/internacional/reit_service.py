...
from cache.cache_manager import CACHE_FILE_REIT, get_data_from_cache
...
def _convert_investidor10_reit_data(json_ticker_page, json_dividends_data, info_names):
    ...
    ALL_INFO = {
        ...
        'debit': lambda: text_to_number(balance['long_term_debt'], should_convert_thousand_decimal_separators=False),
        'dy': lambda: text_to_number(balance['dy'], should_convert_thousand_decimal_separators=False),
        'ebit': lambda: text_to_number(balance['ebit'], should_convert_thousand_decimal_separators=False),
        'enterprise_value': lambda: None,
        'equity_price': lambda: None,
        'equity_value': lambda: balance['total_equity'],
        'gross_margin': lambda: text_to_number(balance['gross_margin'], should_convert_thousand_decimal_separators=False),
        ...
        'net_margin': lambda: text_to_number(balance['net_margin'], should_convert_thousand_decimal_separators=False),
        'net_profit': lambda: balance['net_income'],
        'net_revenue': lambda: balance['revenue'],
        'payout': lambda: text_to_number(balance['api_info']['common_size_ratios']['dividend_payout_ratio'], should_convert_thousand_decimal_separators=False),
        'pl': lambda: text_to_number(balance['pl'], should_convert_thousand_decimal_separators=False),
        'price': lambda: actual_price,
        'pvp': lambda: text_to_number(balance['pvp'], should_convert_thousand_decimal_separators=False),
        'roe': lambda: text_to_number(balance['roe'], should_convert_thousand_decimal_separators=False),
        'roic': lambda: text_to_number(balance['roic'], should_convert_thousand_decimal_separators=False),
...
def _get_reit_from_investidor10(ticker, info_names):
    try:
        headers = {
        ...
        response = request_get(f'https://investidor10.com.br/reits/{ticker}', headers)
        ...
        converted_data = _convert_investidor10_reit_data(json_ticker_page, json_dividends_data, info_names)
...
def _convert_stockanalysis_reit_data(ticker, initial_page, statistics_page, info_names):
    roa = text_to_number(get_substring(statistics_page, 'ROA)",value:"', '%'), should_convert_thousand_decimal_separators=False)
    net_profit = multiply_by_unit(get_substring(initial_page, 'netIncome:"', '"'), should_convert_thousand_decimal_separators=False)

    avg_annual_dividends = text_to_number(get_substring(statistics_page, 'Dividend Per Share",value:"$', '"'), should_convert_thousand_decimal_separators=False)
    ...
    ALL_INFO = {
        ...
        'avg_price': lambda: text_to_number(get_substring(statistics_page, '200-Day Moving Average",value:"', '"'), should_convert_thousand_decimal_separators=False),
        'beta': lambda: get_substring(statistics_page, 'Beta (5Y)",value:"', '"'),
        'cagr_profit': lambda: None,
        'cagr_revenue': lambda: None,
        'debit': lambda: multiply_by_unit(get_substring(statistics_page, 'Debt",value:"', '"'), should_convert_thousand_decimal_separators=False),
        'dy': lambda: text_to_number(get_substring(statistics_page, 'Dividend Yield",value:"', '%'), should_convert_thousand_decimal_separators=False),
        'ebit': lambda: multiply_by_unit(get_substring(statistics_page, 'EBIT",value:"', '"'), should_convert_thousand_decimal_separators=False),
        'enterprise_value': lambda: multiply_by_unit(get_substring(statistics_page, 'Enterprise Value",value:"', '",'), should_convert_thousand_decimal_separators=False),
        'equity_price': lambda: None,
        'equity_value': lambda: None,
        'gross_margin': lambda: multiply_by_unit(get_substring(statistics_page, 'Gross Margin",value:"', '%'), should_convert_thousand_decimal_separators=False),
        ...
        'liquidity': lambda: text_to_number(get_substring(statistics_page, 'Average Volume (20 Days)",value:"', '"'), should_convert_thousand_decimal_separators=False),
        #'liquidity': lambda: get_substring(initial_page, 'v:', '",'),
        'management_fee': lambda: None,
        'market_value': lambda: multiply_by_unit(get_substring(statistics_page, 'Market Cap",value:"', '"'), should_convert_thousand_decimal_separators=False),
        ...
        'net_margin': lambda: multiply_by_unit(get_substring(statistics_page, 'Operating Margin",value:"', '%'), should_convert_thousand_decimal_separators=False),
        'net_profit': lambda: net_profit,
        'net_revenue': lambda: multiply_by_unit(get_substring(initial_page, 'revenue:"', '"'), should_convert_thousand_decimal_separators=False),
        'payout': lambda: text_to_number(get_substring(statistics_page, 'Payout Ratio",value:"', '%'), should_convert_thousand_decimal_separators=False),
        'pl': lambda: get_substring(initial_page, 'peRatio:"', '"'),
        'price': lambda: get_substring(initial_page, 'cl:', ','),
        'pvp': lambda: None,
        'roe': lambda: text_to_number(get_substring(statistics_page, 'ROE)",value:"', '%'), should_convert_thousand_decimal_separators=False),
        'roic': lambda: text_to_number(get_substring(statistics_page, 'ROIC)",value:"', '%'), should_convert_thousand_decimal_separators=False),
        'sector': lambda: get_substring(initial_page, 'Sector",v:"', '",'),
        'total_issued_shares': lambda: multiply_by_unit(get_substring(initial_page, 'sharesOut:"', '"'), should_convert_thousand_decimal_separators=False),
        'total_real_state': lambda: None,
        'type': lambda: 'REIT',
        'vacancy': lambda: None,
        'variation_12m': lambda: text_to_number(get_substring(statistics_page, '52-Week Price Change",value:"', '%'), should_convert_thousand_decimal_separators=False),
        'variation_30d': lambda: None,
    }
...
def _get_reit_from_stockanalysis(ticker, info_names):
    try:
        headers = {
            'accept': '*/*',
            'accept-language': 'pt-BR,pt;q=0.9,en-US;q=0.8,en;q=0.7',
        ...
        converted_data = _convert_stockanalysis_reit_data(ticker, initial_page, statistics_page, info_names)
...
def _get_reit_from_all_sources(ticker, info_names):
    data_stockanalysis = _get_reit_from_stockanalysis(ticker, info_names)
    ...
    data_investidor_10 = _get_reit_from_investidor10(ticker, missing_stockanalysis_infos or info_names)
...
def _get_data_from_sources(ticker, source, info_names):
    SOURCES = {
        VALID_REIT_SOURCES['STOCKANALYSIS_SOURCE']: _get_reit_from_stockanalysis,
        VALID_REIT_SOURCES['INVESTIDOR10_SOURCE']: _get_reit_from_investidor10
    }

    fetch_function = SOURCES.get(source, _get_reit_from_all_sources)
    return fetch_function(ticker, info_names)

def get_reit_data(ticker, source, info_names, can_use_cache):
    cached_data = get_data_from_cache(ticker, CACHE_FILE_REIT, info_names, can_use_cache)
